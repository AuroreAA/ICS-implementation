...
The source code of the ```Julia``` package ```ICSTools``` is available at our GitHub Repository:  [ICSTools](https://github.com/valentint/ICSTools.jl). 
...
To run the code from this tutorial you will need to install ```Julia```. The recommended way of installing ```Julia``` is based on ```juliaup``` which (on Windows) can be found [here](https://apps.microsoft.com/detail/9njnww8pvkmn?hl=fi-FI&gl=FI).
Further you will need an IDE for more comfortable running of the code - the preferred one is [VSCode](https://code.visualstudio.com/) with installed ```Julia``` plugin which can be obtained from [here](https://marketplace.visualstudio.com/items?itemName=julialang.language-julia).

The ```R Markdown``` file of the tutorial could be run in ```RStudio``` but you will need also ```R``` installed.

In [this GitHub repository](https://github.com/AuroreAA/ICS-implementation) is available also a [Pluto](https://plutojl.org/) [notebook](https://github.com/AuroreAA/ICS-implementation/blob/main/Julia/ICS.Pluto) containing a part of the tutorial. 

All the code chunks of the repository are extracted into a [```Julia``` script](https://github.com/AuroreAA/ICS-implementation/blob/main/Julia/Julia_ICS_tutorial.jl).
...
With the function ```fit_predict!()``` we will fit the ICS model and transform the data - equivalent to 
the function ```ICS-S3()``` from the ```R``` package ```ICS```.
Alternatively, this can be done in two separate steps: ```fit!()``` and ```predict!()```, 
as is usually done in machine learning pipelines.
...
For convenience, to look at the data taking the grouping by species and by sex into account we convert the categorical arrays into string arrays.
